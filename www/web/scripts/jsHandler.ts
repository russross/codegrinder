...
class JavaScriptWorker {
  ...
  runningJavaScript = false;

  readonly #callbacks: JavaScriptCallbacks;
  #finishJavaScript: (() => void) | undefined;
  ...
  #handleMessage(data: unknown): void {
    ...
    switch (data.type) {
      case "finished":
        this.#finishJavaScript?.();
        this.#finishJavaScript = undefined;
        return;
      case "failed":
  ...
  async runJavaScript(fileSystem: FileSystem, code: string): Promise<void> {
    await this.ready;
...
class JavaScriptRunner {
  ...
  async runJavaScript(fileSystem: FileSystem, code: string): Promise<void> {
    ...
    await this.#worker.runJavaScript(fileSystem, code);
  }
