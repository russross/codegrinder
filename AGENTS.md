...
- The production server lives under `server/` as the Rust `codegrinder` crate. The old Python server is kept only for port comparison under `pyserver/`.
...
- Stop, start, restart, and check the local CodeGrinder server with `doas rc-service codegrinder ...`; do not launch ad hoc background server processes.
