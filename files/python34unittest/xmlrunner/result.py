#
# This is a fork of https://github.com/pycontribs/xmlrunner
# Changes:
# 1. Dump all output into test_details.xml instead of file/testsuite
# 2. testsuites element includes totals for tests, failures, errors, skipped
...
class _XMLTestResult(_TextTestResult):
    ...
    def generate_reports(self, test_runner):
        ...
        # TODO: get total errors, failures, skipped, tests
        # and add them to the testsuites element
        errors, failures, skipped, count = 0, 0, 0, 0
        ...
        for suite, tests in all_results.items():
            ...
            count += int(testsuite.getAttribute('tests'))

        parentElement.setAttribute('errors', str(errors))
        parentElement.setAttribute('failures', str(failures))
        parentElement.setAttribute('skipped', str(skipped))
        parentElement.setAttribute('tests', str(count))
        xml_content = doc.toprettyxml(
            indent='    ',
            encoding=test_runner.encoding
        )
        filename = path.join(test_runner.output, 'test_detail.xml')
