...
func checkOAuthSignature(w http.ResponseWriter, r *http.Request) {
	...
	if expected == "" {
		logw.Printf("Missing oauth_signature form field")
	...
	// verify it
	if sig != expected {
		logw.Printf("Signature mismatch: got %s but expected %s", sig, expected)
		w.WriteHeader(http.StatusUnauthorized)
	}
...
func saveGrade(db *sql.Tx, commit *Commit) error {
	...
	if asst.GradeID == "" {
		logw.Printf("cannot post grade for assignment %d user %d because no grade ID is present", asst.ID, asst.UserID)
		return nil
	}
	if asst.OutcomeURL == "" {
		logw.Printf("cannot post grade for assignment %d user %d because no outcome URL is present", asst.ID, asst.UserID)
		return nil
	}
	...
	for _, step := range steps {
		...
		if step.ID == commit.ProblemStepID {
			if commit.ReportCard.Passed {
				...
				score += partial
				//logd.Printf("passed %d/%d on this step", passed, len(commit.ReportCard.Results))
			}
			foundCurrent = true
